...
//talvez a parte mais importante do programa
    for i=1:n-1
        t(i,:)=h*i
        phi(t(i),y(i+1,:))= dydt (t(i+h),y(i+1,:))
        y(i+1,:) = y(i,:)+h*phi(t(i),y(i+1,:)) //METODO DE EULER IMPLICITO
        
        exato(i+1,:)= 2*i*h+8*exp(-i*h/2) //Esta eh a função exata! Aqui calcula-se o valor exato da funcao dentro deste loop para cada iteração
    end
//4) SAIDAS DA FUNCAO
...
[m,t,exato,y,e,razao, logaritmo_r] = eulersys(dydtsys,[0,5],8,1)
